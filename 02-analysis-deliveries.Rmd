...
I'm joining various dataframes together, removing unnecessary columns along the way. Some values were calculated from data where doctors were suppressed, and so may be NA.

```{r blurbs_2019}
# Import maternal levels
maternal_levels <- read_csv("resources/maternal-levels.csv") %>% 
  mutate(
    THCIC_ID = as.character(THCIC_ID)
  )


blurbs_2019 <- providers_list %>% 
  rename(PROVIDER_NAME = PROVIDER_NAME_CLEANED) %>% 
