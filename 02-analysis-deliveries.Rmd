...
providers_cleaned <- read_rds("data-processed/providers_cleaned.rds")
...
del_csec_epi_supp <- del_csec_epi %>% 
  mutate(
...
```{r supp_rate_hosp}
supp_rate_hosp <- del_csec_epi_supp %>% 
...
Total deliveries after filtering years with fewer than 30 deliveries. 

```{r deliveries_yr}

# group and count
deliveries_yr <- del_csec_epi_supp %>% 
...
```{r csec_rate_hosp}
csec_rate_hosp <- del_csec_epi_supp %>% 
...
```{r csec_hosp_rate_yr}
csec_hosp_rate_yr <- del_csec_epi_supp %>% 
...
```{r epi_rate_hosp}
epi_rate_hosp <- del_csec_epi_supp %>% 
...
```{r epi_rate_hosp_yr}
epi_rate_hosp_yr <- del_csec_epi_supp %>% 
...
```{r epicsec_rate_hosp}
epicsec_rate_hosp <- del_csec_epi_supp %>% 
  mutate(
...
This uses all deliveries. Hospital exluded if fewer than 30 deliveries a year.

```{r epicsec_rate_hosp_yr}
# prep data for table
epicsec_rate_hosp_yr <- del_csec_epi_supp %>% 
  mutate(
    EPICSEC = if_else((EPI == T | PRCSECP) == T, T, F)
  ) %>% 
...
```{r mediciad_2019}
mediciad_2019 <- del_csec_epi_supp %>% 
  # filter for 2019
  filter(
    YR == 2019
  ) %>% 
