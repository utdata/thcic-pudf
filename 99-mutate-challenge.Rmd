...
the_data <- as.data.table(starwars)
the_data <- the_data[, .(name, eye_color)]
myresults <- the_data[, mycol := any(as_vector(.SD) %chin% dt_my_values), by = seq_len(nrow(the_data))]

myresults %>% count(mycol)
```
