...
> There are issues here with 2019 data. There are zero Cesarean codes in my data, and I need to figure out why.
...
We have enough results to not define as "per 1,000 deliveries". We are using a simple rate: Primary Cesarean / All uncomplicated deliveries.
...
```{r}
del %>% 
  filter(str_detect(PROVIDER_NAME, "Laredo") & YR == "2019")
```
...
## Code records for numberator

The AHRQ spec defines two ways to find this. We use the first listed:

"MS-DRG codes for Cesarean delivery (PRCSE2G* ) without any-listed ICD-10-PCS procedure codes for hysterotomy (PRCSE2P)."

How we handle this:

- Create a cesarean column `PRCSE2G` based on MS-DRG codes. This is an easy `if_else`.
- Create a column `PRCSE2P` that are hysterotomy cases. This is more complicated because we have to look through each SURG_PROC column for any value in the prcse2p_list.
...
## Laredo hospitals: Primary Cesarean rate
...
Some testing for Laredo in 2019
...
### Table of Laredo hospitals Primary Cesarean Rates by year
...
### Chart of Laredo hospitals Primary Cesarean Rates by year
