...
This analysis has enough results to avoide defining as "per 1,000 deliveries". I use a simple rate: Primary Cesarean / All uncomplicated deliveries.
...
I start with data that has already been through some "denominator" exclusions, including filtered for PRCSECD codes and any missing data. This was handled in `01-process-loop`, so I import that resulting file. Important to note with this analysis is the imported cases _DO_ have patients with previous cesareans, so those will have to be filtered out in a later step.
...
### 2019 quarter unfiltered for testing
# del <- read_tsv("data-raw/PUDF 1Q2019 tab-delimited/PUDF_base1_1q2019_tab.txt")
...
- Create a primary Cesarean column `PCSEC` based on Cesarean geing TRUE and hysterotomy being FALSE.
...
## Primary Cesarean rate Laredo hospitals
...
pcsec_rate_yr_all %>% arrange(YR) %>% head()
```
...
### Pivot to compare Primary Cesarean rates by year

```{r pcsec_rate_yr_table}
...
## Searchable table: Primary Cesarean rates by year

```{r pcsec_rate_yr_dtable}
pcsec_rate_yr_all %>% datatable()
```
