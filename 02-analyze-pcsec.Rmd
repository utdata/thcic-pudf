...
### Table: Cesarean rate by hospital, combined years

Get Primary Cesarean rate by hospital from all years combined. Hospitals with fewer than 100 deliveries excluded.
...
Hospitals with fewer than 100 total deliveries excluded.
...
### Primary Cesarean rate and count by year, Laredo

> This is unfinished

For this, perhaps make a stacked bar chart that shows the Total deliveries, with a segment for primary cesareans on the y axis, and yr or quarter on the x axis. (However, that would not show the counts for reporting)

Better would be a table with two levels of names_from, grouped first by year, then by PC_CNT and TOTAL.

```{r}
pcsec_rate_yr_all %>% 
  filter(
    str_detect(PROVIDER_NAME, "Doctors Hospital-Laredo")
  ) %>% 
  select(YR, PROVIDER_NAME, TOTAL) %>% 
  pivot_wider(names_from = c(YR), values_from = c(TOTAL))
```
...
## Doctors at Laredo hospitals

### How many doctors delivering babies per hospital
...
del_pcsec %>% 
  ...
  group_by(YR, ATTENDING_PHYSICIAN_UNIF_ID) %>% 
  summarise(CNT = n()) %>% 
  # filter(CNT >= 10) %>%  # filters docs with less than 10 deliveries
  pivot_wider(names_from = YR, values_from = CNT)
```

It is interesting that there were way more doctors delivering in 2016 than after, even if they had just done a few deliveries.
...
del_pcsec %>% 
  ...
  group_by(PROVIDER_NAME, ATTENDING_PHYSICIAN_UNIF_ID) %>% 
  summarise(CNT = n())
```
