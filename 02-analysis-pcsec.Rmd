...
A look at episitomies by the county of the patient. Note the hospital could be elsewhere.
...
This table excludes fewer than 10 cases in a county in a given year, which reduces the number of available counties.
...
## By First Payment Source

Set a medi-value: MA, MB, MC
FIRST_PAYMENT_SRC

```{r set_medi}
ahrq_pcsec %>% 
  count(FIRST_PAYMENT_SRC)
```


```{r ahrq_pcsec_rate_payment}
ahrq_pcsec_rate_county <- ahrq_pcsec %>% 
  group_by(PAT_COUNTY, PCSEC) %>% 
  summarize(CNT = n()) %>% 
  ### join for county names
...
- ahrq_pcsec_rate_hosp_yr: Grouped by PROVIDER_NAME and YR. Aggregations are NPCSEC_CNT (Count not primary Cesarean), PCSEC_CNT (Count primary Cesarean), TOTAL, PCRATE (Primary Cesarean rate).
- ahrq_pcsec_rate_hosp_yr: Grouped by PAT_COUNTY, COUNTY. Aggregations are NPCSEC_CNT (Count not primary Cesarean), PCSEC_CNT (Count primary Cesarean), TOTAL, PCRATE (Primary Cesarean rate).
